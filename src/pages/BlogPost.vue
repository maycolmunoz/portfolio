...
import { getPost, getPostContent, blogPosts } from '@/content/blog'
...
const currentIndex = computed(() => sortedPosts.value.findIndex((p) => p.slug === slug))
const prevPost = computed(() => sortedPosts.value[currentIndex.value + 1])
const nextPost = computed(() => sortedPosts.value[currentIndex.value - 1])
...
usePageMeta(
  ...
  { path: post.value ? `blog/${post.value.slug}` : 'blog' },
)
...
<template>
  <div class="max-w-3xl mx-auto px-4 xl:px-0 py-8">
    ...
    <template v-if="post">
      ...
      <nav class="mt-12 pt-6 border-t-2 border-dashed border-border flex flex-col gap-4">
        <div class="flex items-center justify-between gap-4">
          <RouterLink
            v-if="prevPost"
            :to="`/blog/${prevPost.slug}`"
            class="inline-flex items-center gap-2 font-body text-sm text-text-secondary hover:text-primary transition-colors duration-200 min-w-0"
          >
          ...
          <RouterLink
            ...
            class="inline-flex items-center gap-2 font-body text-sm text-text-secondary hover:text-primary transition-colors duration-200 min-w-0 ml-auto"
          >
        ...
        <div
          class="flex items-center justify-between gap-4 border-t border-dashed border-border pt-4"
        >
          <RouterLink
            to="/blog"
